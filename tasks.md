...
- [x] **Export preview uses real builders (gap fix 2026-07-16).** `shared/trackers/describeExportPlan.ts` calls ClickUp shared-core (`buildEpicListBody` / `buildFeatureTaskBody` / `buildStoryTaskBody`) and surfaces their actual tags / markdown fields. **ADO + Jira remain explicit hand-typed fallbacks** until their shared-core extractions land — not silently presented as equally “real.” Covered by `describeExportPlan.test.ts` equality vs `buildStoryTaskBody`.
...
- [x] **Batch notes + isolation (audit Round 1, 2026-07-16).** `GET /api/generations/:id/notes` batches all notes; App loads once with collab (no per-story N+1). Isolation proven by `collab.isolation.test.ts` (B → 404 on batch notes/collab mutate). Metrics aggregates proven by `metrics/summary.test.ts` (user-scoped counts + generate→export JOIN).
- [x] **Session prune + interaction rate limit (audit Round 2, 2026-07-16).** `npm run audit:prune` also `DELETE FROM sessions WHERE expires_at < now()`; opportunistic prune on login. Metrics + collab share `RATE_LIMIT_INTERACTION_PER_HOUR` (default 120) via `interactionLimit`.
- [x] **Collapsible collab + metrics polish (audit Round 3, 2026-07-16).** StoryCollabPanel collapsed-by-default; metrics COUNTs use one FILTER query; avg Gemini `metadata.durationMs` measured separately from full-cycle generate→export; `session.prune.test.ts` proves expired-only DELETE.
